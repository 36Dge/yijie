...
fn inspect()->Result<(),&'static str>{
 ...
 for query in [
 "SELECT 'run='||r.run_id||';plan='||r.plan_id||';trigger='||r.trigger_source||';delivery='||r.delivery_state||';outcome='||r.native_outcome||';grant='||r.grant_id||';op='||r.operation_id FROM chat_scheduled_runs r",
 ...
 "SELECT 'plan='||plan_id||';state='||state||';revision='||revision FROM chat_scheduled_plans",
