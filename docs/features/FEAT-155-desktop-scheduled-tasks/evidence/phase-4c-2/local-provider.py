...
class Provider(http.server.BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        item = {'id': 'message_automatic_local_1', 'type': 'message', 'role': 'assistant', 'phase': 'final_answer', 'status': 'completed', 'content': [{'type': 'output_text', 'text': '4C2 本机合成自动验证完成', 'annotations': []}]}
        events = [
            {'type': 'response.created', 'response': {'id': 'response_automatic_local_1', 'status': 'in_progress', 'output': []}},
            {'type': 'response.output_item.added', 'output_index': 0, 'item': item},
            {'type': 'response.output_item.done', 'output_index': 0, 'item': item},
            {'type': 'response.completed', 'response': {'id': 'response_automatic_local_1', 'status': 'completed', 'output': [item], 'usage': {'input_tokens': 1, 'output_tokens': 1, 'total_tokens': 2}}},
        ]
