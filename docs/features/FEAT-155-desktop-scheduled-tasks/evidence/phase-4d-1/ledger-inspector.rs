...
fn inspect()->Result<(),&'static str>{
 ...
 for query in [
 "SELECT 'native_binding='||json_object('run_id',r.run_id,'plan_id',r.plan_id,'conversation_id',b.conversation_id,'local_turn_id',b.local_turn_id,'agent_session_id',s.agent_session_id,'thread_id',s.runtime_thread_id,'turn_id',t.runtime_turn_id) FROM chat_scheduled_runs r JOIN chat_scheduled_run_bindings b ON b.run_id=r.run_id JOIN chat_sessions s ON s.id=b.conversation_id JOIN chat_turns t ON t.id=b.local_turn_id WHERE r.run_id IN ('01a0cd55-08da-7942-b047-e2f1ed71ef53','01a0cd75-947d-7842-b424-1ac09d3f38d0')",
 ...
 if v>=26 {
  let mut st=c.prepare("SELECT 'clock='||json_object('run_id',run_id,'format_version',format_version,'fact',json(fact_json),'conflicts',conflicts,'pending',pending,'attempts',attempts,'diagnostic',diagnostic) FROM chat_scheduled_timing ORDER BY run_id").map_err(|_|"clock query unavailable")?;
  for row in st.query_map([],|r|r.get::<_,String>(0)).map_err(|_|"clock facts unavailable")? {println!("{}",row.map_err(|_|"clock fact unavailable")?);}
 }
 Ok(())
}
