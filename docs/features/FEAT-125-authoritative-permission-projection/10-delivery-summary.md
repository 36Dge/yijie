...
> 功能交付完成记录。S1—S4 与 S5A 已推送并远端核验；G3-NP-LOCAL 三仓静态实现/门禁、
> local stack/live realm、HTTPS synthetic user provisioning、offline ready、synthetic API bootstrap 与
> core online 已完成。因此 G3-NP-LOCAL PASS；S5B 仅具备单独审批条件、仍未批准；S5B
> generated consumer/store、S6 UI、S7 E2E、发布和生产栏位继续保持 `NOT RUN / Not
> delivered`。
...
- 原目标是否达成：部分；已完成 A1—A7、S1/S2 Contracts remote candidate、S3 API
  ...
  dedicated API/core-online 链。尚无 Desktop generated consumer/store/UI、S7 完整 auth lifecycle、
  跨仓业务 E2E 或生产链路。
- 最终范围与非目标：见 00—07；G1/G2 于 2026-07-31 Passed，G2A 于 2026-08-01 Passed。
- 交付状态：`S1—S4 Remote / S5A Remote / G3-NP-LOCAL PASS / S5B Eligible but Not Approved / Not delivered`。
...
| yijie-desktop G3-NP-LOCAL | repository commit / no release | local CA/Keychain environment-binding implementation | `446b4d608546fca8f53f4582201d6b43ef6f762d`；origin/develop verified | Git commit | S5B exact contract pin not formed |
...
| G3-NP-LOCAL dedicated API/online | PASS | exit 0 | strict local-only CA pin；discovery/JWKS/callback、health/ready、两个 unauth 401、Tasks edge+direct 404 PASS；S5B not approved |
...
|---|---|---|---|
| 授权/租户/审计/脱敏 | synthetic S3/S4 + local G3 candidates | PASS for foundation/producer/static native boundary/bootstrap/offline readiness：fail-closed reads、cross-tenant FK、append-only、逐请求 membership/RBAC projection、稳定错误、token storage/IPC/transport allowlist、bootstrap audit/revision、secret-free diff、CA/config validation；dedicated API startup/readiness、passing online rerun、S5B UI/cross-repo pending | 08 verification report + Desktop/Infra owning-repo evidence |
...
|---|---|---|---|---|
| direct IdP JWT/native auth/tenant/RBAC | API S3/S4 + Desktop S5A + G3 live realm/offline/discovery/JWKS/callback PASS；API local-only explicit CA pin implemented，core online PASS；browser/bearer/Keychain/cross-repo 未验证 | 段成威 | G2A/S3/S4/S5A/A7 scope approved；G3 passed | approve minimum CA path→G3 online；then separate S5B→S7/G5 |
...
| G3 local IdP/infra | local vendor/issuer/client/JWKS/TLS/API origins 已固定；static gates/local stack/live realm/HTTPS user provisioning/offline ready/discovery/JWKS/callback PASS；API local-only explicit CA pin implemented，core online PASS | 段成威 | A7 scope only | separate CA approval + G3 completion |
...
|---|---|---|---|---|
| G3-NP-LOCAL | 段成威 | PASS | 2026-08-01 | strict explicit CA pin、offline ready、core online 与最终三仓门禁 PASS；S5B 未批准 |
| G6 Delivery Complete | 段成威 | Not approved / feature open | 2026-08-01 | S1—S4 与 S5A remote；G3 passed；S5B—S8/G4/G5/G6 Pending |
