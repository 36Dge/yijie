...
S4 + S5A
  ...
  → G3-NP-LOCAL Keycloak/PostgreSQL/Caddy + bootstrap + explicit CA（static/local stack/offline ready/bootstrap complete）
        ...
        → S5B Desktop exact pin + permission client/store（Complete: 5c4600f8308d...）
...
| Consumer | yijie-desktop | develop / base `be01cc2d...` → S5A `3798c67d...` → S5B `5c4600f8308d...` remote | native auth boundary and generated adapter/store complete；S6 UI pending | exact contracts candidate pinned in S5B | 段成威 |
...
| S5B | `5c4600f8308d55be5596e7c45215e88c7411f286` | exact contract/generator lock、generated TypeScript、fixed adapter、0/1/multiple tenant state、operation intent、revision/expiry/context validation、memory fail-closed store | generation drift + canonical contract/fault/concurrency/security；frontend 12 files/80 tests；Rust 36 tests；lint/build/docs/debug app+dmg/audits/peers PASS | S5B-REV-001—004 resolved；open P0/P1/P2=0；S5A-REV-OPEN-001 remains outside S5B | Complete / remote verified；feature off；no S6/UI/Tasks/Rust lifecycle/production change |
