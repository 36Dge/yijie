...
let targetPID: pid_t = 4642
let expectedExecutable = "/Users/jack/Downloads/Personal_Info/CrossBSD/yijie-desktop/src-tauri/target/debug/yijie-desktop"
let evidenceRoot = "/Users/jack/Downloads/Personal_Info/CrossBSD/yijie/docs/features/FEAT-153-coze-workflow-foundation/evidence/step6-d4-20260913-1041/"
...
@MainActor func run() async throws {
    ...
    case "press", "focus", "fill":
        ...
        func find(_ item: AXUIElement, _ depth: Int) {
            ...
            if (args[0] != "press" ? ["AXTextField", "AXTextArea"] : ["AXButton", "AXLink", "AXDisclosureTriangle", "AXCheckBox"]).contains(role) && string(item, kAXTitleAttribute) == args[1] { matches.append(item) }
            for child in attribute(item, kAXChildrenAttribute) as? [AXUIElement] ?? [] { find(child, depth + 1) }
        }
        find(root, 0)
        try require(matches.count == 1, "Expected one observed matching button/link")
        if args[0] == "fill" {
