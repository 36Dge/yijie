...
def main():
    ...
    with os.fdopen(fd, 'r') as lock:
        ...
        expected = {
            'revision': resource['revision'] == '7684601577516040192',
            'name': resource['name'] == 'FEAT-153 Dev App 验收 20260912-1840',
            ...
            'text_position': by_id.get('200001', {}).get('position') == {'x': 405, 'y': 91},
            'end_position': by_id.get('900001', {}).get('position') == {'x': 760, 'y': 160},
            'prefix': by_id.get('200001', {}).get('prefix') == '真实 Desktop 到期重连：',
        ...
        result = {'schema_version': 1, 'observation': 'dev_final_app_single_resource_readonly', 'completed_read': True, 'workflow_id': WID, 'run_epoch': EPOCH, 'recorded_at': now(), 'observed_revision': resource['revision'], 'snapshot_note': 'Final dev resource observation at the stated times; PostgreSQL and MySQL are separate read-only snapshots. Root paused resource edits during the read. This does not establish full UI/session-close qualification.', 'expected_dev_resource_checks': expected, 'postgres': pg, 'mysql': mysql, 'cross_database': {'scope_identity_match': pg['facts']['resource'][0]['coze_user_id'] == resource['creator_id'] and pg['facts']['resource'][0]['coze_space_id'] == resource['space_id'], 'shared_operations': compared, 'postgres_only': sorted(pg_ops.keys() - my_ops.keys()), 'mysql_only': sorted(my_ops.keys() - pg_ops.keys())}, 'session_close': 'NOT OBSERVED: API sessions are memory-only; database rows do not prove revocation', 'script_sha256': hashlib.sha256(Path(__file__).read_bytes()).hexdigest()}
