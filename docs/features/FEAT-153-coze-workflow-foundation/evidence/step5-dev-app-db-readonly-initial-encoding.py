...
SELECT 'resource', JSON_OBJECT('workflow_id',CAST(m.id AS CHAR),'name',m.name,'space_id',CAST(m.space_id AS CHAR),'creator_id',CAST(m.creator_id AS CHAR),'app_id',CAST(m.app_id AS CHAR),'latest_version',m.latest_version,'revision',d.commit_id,'updated_at_ms',d.updated_at,'modified',d.modified,'test_run_success',d.test_run_success,'canvas',d.canvas,'principal_scope',p.scope_key) FROM workflow_meta m JOIN workflow_draft d ON d.id=m.id JOIN yijie_workflow_local_principal p ON p.user_id=m.creator_id AND p.space_id=m.space_id WHERE m.id={WID} AND m.app_id=0 AND m.deleted_at IS NULL AND d.deleted_at IS NULL AND p.scope_key='{SCOPE}';
...
unset password
mysql --defaults-file="$option_file" --no-login-paths --batch --raw --skip-column-names --connect-timeout=5 coze_workflow_local <<'APP_READ_ONLY_SQL'
...
def main():
    ...
    with os.fdopen(fd, 'r') as lock:
        ...
        with os.fdopen(outfd, 'w') as out:
            ...
            out.flush()
            os.fsync(out.fileno())
        print(json.dumps({'read_completed': True, 'workflow_id': WID, 'observed_revision': resource['revision'], 'node_count': len(nodes), 'edge_count': len(graph['edges']), 'pg_operations': len(pg_ops), 'mysql_operations': len(my_ops), 'shared_receipts_match': all(i['fields_match'] for i in compared), 'evidence': str(DEST)}, ensure_ascii=False))
