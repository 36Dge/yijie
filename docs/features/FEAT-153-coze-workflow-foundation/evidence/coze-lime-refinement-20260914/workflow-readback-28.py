...
EXPECTED_MANIFEST = '6cf9228ae89480f94197ff48193fdaa0e0580a65cae893ae8c93001489b12def'
EXPECTED_SOURCE = 'b69117dec6ff31092f6078dad3281d5ca73270042f2e6ebf05a43dd5cebe1fdf'

def main():
    stage = sys.argv[1] if len(sys.argv) == 2 else ''
    r.c.require(stage in ['saved-draft28', 'final-runs28', 'final28', 'after-normal-restart28'], 'Explicit snapshot stage required')
    destination = HERE / (stage + '.json')
    r.c.require(not destination.exists(), 'New evidence file required')
    activation = json.loads((HERE / 'activation28.json').read_text())
    r.c.require(activation['manifest_sha256'] == EXPECTED_MANIFEST and activation['source_digest'] == EXPECTED_SOURCE, 'Expected canonical 28 activation differs')
    ...
    with os.fdopen(fd, 'r') as lock:
        ...
        r.c.require(values == ['轻盈青柠：{{input}}'], 'Exact synthetic text prefix differs')
