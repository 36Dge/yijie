...
let targetPID: pid_t = 11566
let expectedExecutable = "/Users/jack/Downloads/Personal_Info/CrossBSD/yijie-desktop/src-tauri/target/debug/yijie-desktop"
...
@MainActor func run() async throws {
    ...
    case "press", "press-outer", "open-cancel", "focus", "fill":
        ...
        if args[0] == "fill" {
            ...
            if args[0] == "open-cancel" {
                var cancelObserved = false
                for _ in 0..<10 {
                    try await Task.sleep(nanoseconds: 10_000_000)
                    var candidates: [AXUIElement] = []
                    var examined = 0
                    func inspect(_ item: AXUIElement, _ depth: Int) {
                        guard depth < 20 && examined < 1600 else { return }; examined += 1
                        if string(item, kAXRoleAttribute) == "AXButton" && string(item, kAXTitleAttribute) == "取消打开" && string(item, kAXEnabledAttribute) == "1" { candidates.append(item) }
                        for child in attribute(item, kAXChildrenAttribute) as? [AXUIElement] ?? [] { inspect(child, depth+1) }
                    }
                    inspect(root, 0)
                    if candidates.count == 1 {
                        try require(AXUIElementPerformAction(candidates[0], kAXPressAction as CFString) == .success, "Observed cancel button declined")
                        cancelObserved = true; break
                    }
                }
                try json(["pid": targetPID, "opened_control": args[1], "normal_pending_cancel_observed_and_pressed": cancelObserved, "artificial_delay_or_fault": false])
            }
        }
    case "click":
