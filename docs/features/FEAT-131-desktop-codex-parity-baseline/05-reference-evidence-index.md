...
| EVID-SCHEMA-004 | Command/FileChange approval、MCP elicitation、permission approval、dynamic tool server requests | `yijie-codex/.yijie/schemas/app-server/generated-json-schema/ServerRequest.json:1796-1846`、`:1873-1946` | 只保留 schema 事实；GS-005 / FEAT-137 仅涵盖 Command approval，FileChange、permission 与 MCP elicitation 均不属于本 Feature |
| EVID-SCHEMA-005 | `item/tool/requestUserInput` 在 non-experimental schema 中存在，但带 EXPERIMENTAL 协议注释 | `yijie-codex/codex-rs/app-server-protocol/src/protocol/common.rs:1474-1478`；generated `ServerRequest.json:1848-1871` | 保持 `requires Host/Contracts projection`，不归因为 `experimentalApi=false`，且明确不属于 FEAT-137 |
...
| EVID-F144-001 | Sorftime原生Tool本地交付 | FEAT-144当前四文件、21–24、原生拒绝/重开及来源适用性证据、分账台账；早期失败与备选保留为历史 | E真实completed/414ms及数据对照、模式停用/复原；F最终来源原生拒绝failed/0ms及普通重开；元数据14/20、文本9/13、业务保守4/10；AC-004用户排除，最终D4输出见24 |
| EVID-F137-D0-001 | CAP-019 exact local Command approval Owner decision：stable on-request/read-only、唯一 Git repository check、workspace-root cwd、accept_once/cancel、120 秒 TTL、pending memory-only、content-free audit 128/session | FEAT-137 four-file package；Desktop Epic `FEAT-137-desktop-approval-interaction.md` | D0 `PASS`；Contracts/Host/Desktop implementation、conformance、real-runtime 与 D4 均 `NOT RUN`；本批真实调用 0 |
| EVID-F137-CONTRACT-CANDIDATE-001 | 独立协商 v6 approval requested/resolved、pending snapshot、decision API、stable errors、Runtime compatibility projection 与 generated consumers 的 working-tree source candidate | `yijie-contracts/docs/reviews/FEAT-137-semantic-review.md`；FEAT-137 四文件包 | source candidate review `PASS`（focused 25/25、安全合规 Node 88/88；branch `feat/feat-137-contracts-v6-approval`，current HEAD/base `87f94c9…`）；dirty 且无 immutable commit/consumer pin，`contract.status`、Host/Desktop/Runtime change、conformance、D4 均 `NOT RUN`；误执行的 archive suite 已排除；真实调用 0 |
| EVID-F137-SOURCE-001 | Contracts v6、Host mapper/pending/decision/deadline、Desktop closed consumer/SQLCipher/inline UI 的最终本地不可变 source chain | Contracts `2e490dea4444ea1e33c2df1a5267b2bff5bfb8e6`；Host `118651804b7f5a7849bc68cdf29d88c74a21f8a1`；Desktop `918bd26b04a32ab20433e2ea991cc0a4ea282c34`；FEAT-137 delivery log §10–13 | clean-tree source conformance `PASS`；首次 freeze 的 commit→late-ack P1 已以 deterministic deadline/cleanup tests修复；Web 291/291、Rust 19/19；无剩余P0–P2。真实allow/cancel、自然expiry/reconnect、人工视觉与D4 `NOT RUN`；真实调用0 |
...
| GS-004 Tool 成功/原生权限拒绝 | `owner-approved-inference` | Sorftime单工具真实成功、实际原生Prompt拒绝与正常重开完成；业务失败排除，E/F来源及缺字段限制保留，最终D4见24 | EVID-SCHEMA-003/006、EVID-HC-002、EVID-F144-001 | FEAT-144 |
