...
本 Profile 不建立治理切片。FEAT-131 只交付后续 FEAT-132–143 共用的近似设计策略、能力边界、场景和验证入口；完整 Epic 目标仍需后续 Feature 实现。

## 2. 实际改动

| Repository | 模块/文件 | 行为变化 | 原因 |
|---|---|---|---|
| yijie | `docs/features/FEAT-131-desktop-codex-parity-baseline/` | 建立 demo_fast Feature Package、近似参考策略、固定 Runtime 身份、38 项能力矩阵、13 个黄金场景和验证索引 | 为 FEAT-132–143 提供同一治理基线 |
...
- 近似策略固定不等于 Epic 已完成；生产对话行为仍由 FEAT-132–143 分阶段交付。
