...
| S3 | 实现 `/chat` 新建任务入口和轮播 | AC-001/004/007/009 | yijie-desktop | `placeholder-rotation.ts` + test、`useRotatingPlaceholder.ts` + test、`ChatPage.vue`、必要 scoped CSS；`src/styles/variables.css` 只补 Accepted Chat 参考已定义的输入区高度、顶部偏移、字重和边框 token | 发送、附件、草稿持久化、API/Agent/Runtime | S2 | PH tests；`make lint/test/build`；浏览器输入检查 | 回退 S3，保留新 Shell |
