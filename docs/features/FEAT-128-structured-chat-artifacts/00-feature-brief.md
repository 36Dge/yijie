...
- G2A 只授权按依赖启动 S3/S4；Host/Desktop 业务实现、真实 provider、发布和生产门禁仍未通过，不能由 pin conformance 推导为完成。
