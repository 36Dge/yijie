...
- S3/S4/S5 的 G3 slice gate 已通过且范围未扩展；S6A/S6B/S7F/S7A/S7A-REPAIR/S7B/S8A/S8B/S9A 作为独立切片 PASS，均不并入
  G3。S9B-S12、真实 provider、tag、push、release、production 与 G4-G6 仍关闭。

## 2. 实际版本与提交

| Component | Version/status | Full commit | 说明 |
|---|---|---|---|
| Feature package | G3 S3/S4/S5 + S6/S7/S8A/S8B/S9A separate PASS | 本次 `yijie` 文档提交 | governance/evidence only；G3 unchanged；G4 pending |
| Desktop Pattern | Accepted 1.4.0 S9 readiness | `b6f7401c79d5b2356bc45468f14d7fdbb17a855c` | docs-only historical readiness；S9A later passed separately；S9B NOT RUN |
...
| AC-006 report behavior | CONTRACT PASS / S9A NATIVE PASS / S9B NOT RUN | exact report v1/fixtures 与 Desktop conformance repair/projection/canonical JSON save 已通过；renderer/chart/vertical 尚未实现，不记完整 AC PASS |
...
- S9A save 只允许 validated canonical report `.json`、明确用户 intent、dialog 前后双验、native atomic write 与 exact
  report-only residue validation；PDF/Markdown/image derived export 延期。S9B fixed chart mapping 禁止任意 ECharts option/
  formatter/HTML/URL/dynamic code并始终提供 accessible table；当前 ECharts/theme blocker 未解除。
...
| G4 Code Complete | PENDING | N/A | S9B/S10-S11、Markdown/full AC、E2E/full visual/performance/独立 review 未完成 |
...
| S9B report renderer/chart | BLOCKED / NOT RUN | S9A immutable PASS 前置已满足；仍等待单独授权 + exact pinned ECharts dependency/theme/card blocker 解除 |
...
1. 下一步应先关闭 S9B 的 exact pinned ECharts dependency/lockfile、tree-shaken imports、易界 theme/card 与 security/a11y review blocker，再请求单独 S9B 编码授权；不得直接启动 S9B、production page、S10-S12 或 G4。
2. S9B 若需要 native/private schema、Contracts/Host/public pin、pages/stores、未批准的 dependency/config/protocol/CSP/capability/migration、
   derived export、扩大 frozen limits 或改变 S6-S9A 行为，立即停止并重开相应 review。
...
|---|---|---|---|
| Feature package | `yijie/docs/features/FEAT-128-structured-chat-artifacts/` | 段成威 | G3 remains S3/S4/S5；S6/S7/S8A/S8B/S9A separate PASS；S9B-S11 pending |
| Desktop UI Pattern | `yijie-desktop/docs/design/docs/design/05-patterns/14-feat-128-structured-chat-artifacts.md` | 段成威 | Accepted 1.4.0 at `b6f7401c...`；S9A later separate PASS；S9B NOT RUN |
...
正式关闭时间尚未形成。当前准确状态是：`G3 PASS only for S3/S4/S5; S6A, S6B, S7F, S7A, S7A-REPAIR, S7B, S8A, S8B and S9A separate PASS; S9B-S11 pending; real providers closed; G4-G6 not passed`。
