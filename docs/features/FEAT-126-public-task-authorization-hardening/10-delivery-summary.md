...
|---|---|---|---|---|
| Host/Desktop title/raw-reasoning integration | Host producer、Desktop SQLCipher、Rust/IPC/store及Vue纯文本projection已实现并分层通过；full E2E未执行 | 段成威 | flags off；后续仍须分别授权S9–S10验证 | before G4 |
...
- 正式关闭时间：N/A；feature remains active at G2A Re-review Passed / G3-partial boundary。S4–S8B Closure已由Owner接受；S9–S11仍须分别审批。dependency remediation/merge与MiniMax local smoke继续分别单审，tag/publish/deploy不在本期范围。
